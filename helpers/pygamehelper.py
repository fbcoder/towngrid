...
class PygameHelper:
    def __init__(self, size=(640,480), fill=(255,255,255),full=False,programName="Application"):
		pygame.init()        
		if full:
    ...
    def mainLoop(self, fps=0):
        ...
        while self.running:
            pygame.display.set_caption("%s @ %i FPS" %(self.programName,self.clock.get_fps()))
            self.handleEvents()
            self.update()
            self.draw()
            pygame.display.flip()
